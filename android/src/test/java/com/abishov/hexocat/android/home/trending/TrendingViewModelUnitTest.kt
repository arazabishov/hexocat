...
import com.abishov.hexocat.android.common.dispatcher.TestDispatcherProvider
import com.abishov.hexocat.android.common.livedata.captureValues
import com.abishov.hexocat.android.common.rule.TestCoroutineScopeRule
...
class TrendingViewModelUnitTest {
    ...
    fun setUp() {
        ...
        trendingViewModel = TrendingViewModel(testDispatcherProvider, githubService)
    ...
    fun `view model must propagate correct states on failure`() = coroutineScope.runBlockingTest {
        whenever(githubService.search(searchQuery, 32))
            .thenThrow(RuntimeException("test_message"))
    ...
    @Test
    fun `view model must propagate correct states when no items`() =
